...
def main():
    ...
    df = spark.read.csv("/Users/qiangdanlei/Desktop/Toronto,\ Ontario,\ Canada/listings-1.csv",
    ...
    top_average_neigh = df_with_average.withColumn('rank', functions.rank().over(window)).filter(
        functions.col('rank') <= 5).orderBy('neighbourhood_cleansed').withColumn("city", lit("Toronto")).drop(df['name'])
    top_average_neigh = top_average_neigh.withColumnRenamed('neighbourhood_cleansed','neighborhood').toPandas()



    try:
        top_average_neigh.to_sql('backend_topavgscorebyneighbor', con=engine, if_exists='append', index=False)
